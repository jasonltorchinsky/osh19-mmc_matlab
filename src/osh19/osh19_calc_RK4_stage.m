...
% Unexplained discrepancies in original code
stage_in.u(:,:,nz+1) = nz * stage_in.u_tau ...
    - squeeze(sum(stage_in.u(:,:,2:nz), 3));
stage_in.v(:,:,nz+1) = nz * stage_in.v_tau ...
    - squeeze(sum(stage_in.v(:,:,2:nz), 3));

u_tau2 = 1/nz * ( sum((stage_in.u(:,:,2:end)).^2,3) ...
    + (nz*stage_in.u_tau - squeeze(sum(stage_in.u(:,:,2:end),3))).^2 );
utau_vtau = 1/nz * ( sum(stage_in.u(:,:,2:end).*stage_in.v(:,:,2:end),3) ...
    + ( nz*stage_in.u_tau - squeeze(sum(stage_in.u(:,:,2:end),3))) ...
        .* (nz*stage_in.v_tau - squeeze(sum(stage_in.v(:,:,2:end),3))) );
v_tau2 = 1/nz * ( sum((stage_in.v(:,:,2:end)).^2,3) ...
    + (nz*stage_in.v_tau - squeeze(sum(stage_in.v(:,:,2:end),3))).^2 );

% Calculate BT variables
u_tau2    = dealias(u_tau2, 2.0/3.0);
utau_vtau = dealias(utau_vtau, 2.0/3.0);
v_tau2    = dealias(v_tau2, 2.0/3.0);
